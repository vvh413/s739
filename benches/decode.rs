    ...
    for size in [16, 512, 1024, 512 * 1024, 5 * 1024 * 1024].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                let mut encoder =
                    encode::png::PngEncoder::new(image, ExtraArgs::default()).unwrap();
                encoder.write_data(&data).unwrap();
                b.iter(|| {
                    let decoder =
                        decode::png::PngDecoder::new(encoder.image.clone(), ExtraArgs::default())
                            .unwrap();
                    decoder.read_data()
                })
            },
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                let mut encoder =
                    encode::jpeg::JpegEncoder::new(&image_buffer, ExtraArgs::default()).unwrap();
                encoder.write_data(&data).unwrap();
                let image_buffer = encoder
                    .encode_image(s739::options::ImageOptions::default())
                    .unwrap();
                b.iter(|| {
                    let decoder =
                        decode::jpeg::JpegDecoder::new(&image_buffer, ExtraArgs::default())
                            .unwrap();
                    decoder.read_data()
                })
            },
